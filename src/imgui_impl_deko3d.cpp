...
struct ImGui_ImplDeko3d_Data
{
  ...
  dk::SamplerDescriptor *samplerDescriptors = nullptr;
  dk::ImageDescriptor *imageDescriptors   = nullptr;

  PadState pad;
  u64 last_tick = armGetSystemTick();

  DeckoFontType fontType;
};

static u64 disabled_buttons = 0;
...
static void ImGui_LoadSwitchFonts(ImGuiIO &io, ImGui_ImplDeko3d_Data *bd)
{
  ...
  ImWchar ranges[] = { // All languages with chinese included
  ...
  if (bd->fontType & DECKO_FONT_TYPE_SIMPLIFIED_CHINESE)
  {
    io.Fonts->AddFontFromMemoryTTF(
        s_chinese.address, s_chinese.size, 18.0f, &font_cfg,
  ...
  {
    io.Fonts->AddFontFromMemoryTTF(
        t_chinese.address, t_chinese.size, 18.0f, &font_cfg,
  ...
  if (bd->fontType & DECKO_FONT_TYPE_JAPANESE)
  {
    io.Fonts->AddFontFromMemoryTTF(standard.address, standard.size, 18.0f, &font_cfg,
...
void ImGui_ImplDeko3d_Init(DeckoFontType fontType)
{
  ImGuiIO &io = ImGui::GetIO();
...
void ImGui_ImplDeko3d_DisableButtons(u64 buttons)
{
